...
def im_resample(im, subsample_width, subsample_height, resample_width, resample_height):
    ...
    im_subsample = cv2.resize(
        im, (subsample_width, subsample_height), interpolation=cv2.INTER_LINEAR
    )
    return cv2.resize(
        im_subsample, (resample_width, resample_height), interpolation=cv2.INTER_NEAREST
    )
...
def im_quantize(im, n_clusters):
    ...
    im_quant_colorspace = clt.cluster_centers_.astype(np.uint8)[labels].reshape(
        (h, w, 3)
    )
